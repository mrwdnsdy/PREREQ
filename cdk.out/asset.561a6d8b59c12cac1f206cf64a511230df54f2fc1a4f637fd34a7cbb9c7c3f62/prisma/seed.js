...
async function main() {
    ...
    for (const taskData of sortedTasks) {
        ...
        const task = await prisma.task.create({
            data: {
                ...
                roleHours: taskData.roleHours || null
            }
        });
    ...
    const relationships = [
        { pred: '1', succ: '2', type: 'FS' },
        { pred: '2', succ: '3', type: 'FS' },
        { pred: '3', succ: '4', type: 'FS' },
        { pred: '4', succ: '5', type: 'FS' },
        { pred: '5', succ: '6', type: 'FS' },
        { pred: '1.1', succ: '1.2', type: 'FS' },
        { pred: '1.2', succ: '1.3', type: 'FS' },
        { pred: '2.1', succ: '2.2', type: 'FS' },
        { pred: '2.2', succ: '2.3', type: 'FS' },
        { pred: '2.3', succ: '2.4', type: 'FS' },
    ];
    ...
    const createdLabourResources = await Promise.all(labourResources.map(resource => prisma.resource.create({
        data: {
            name: resource.name,
    ...
    const createdEquipmentResources = await Promise.all(equipmentResources.map(resource => prisma.resource.create({
        data: {
            name: resource.name,
            rateFloat: resource.rate,
    ...
    const createdMaterialResources = await Promise.all(materialResources.map(resource => prisma.resource.create({
        data: {
            name: resource.name,
            rateFloat: resource.rate,
    ...
    const createdConsultingResources = await Promise.all(consultingResources.map(resource => prisma.resource.create({
        data: {
            name: resource.name,
            rateFloat: resource.rate,
