...
describe('DependenciesService', () => {
    ...
    beforeEach(async () => {
        ...
        service = module.get(dependencies_service_1.DependenciesService);
    ...
    describe('create', () => {
        ...
        it('should throw BadRequestException for self-link (predecessor = successor)', async () => {
            ...
            await expect(service.create(selfLinkDto)).rejects.toThrow(new common_1.BadRequestException('Successor task cannot be the same as predecessor task (self-link not allowed)'));
        ...
        it('should throw BadRequestException when predecessor task does not exist', async () => {
            mockPrismaService.task.findUnique
                .mockResolvedValueOnce(null)
                .mockResolvedValueOnce(mockTask2);
            await expect(service.create(createDto)).rejects.toThrow(new common_1.BadRequestException('Predecessor task with ID task1 not found'));
            expect(mockPrismaService.taskDependency.create).not.toHaveBeenCalled();
        });
        it('should throw BadRequestException when successor task does not exist', async () => {
            mockPrismaService.task.findUnique
                .mockResolvedValueOnce(mockTask1)
                .mockResolvedValueOnce(null);
            await expect(service.create(createDto)).rejects.toThrow(new common_1.BadRequestException('Successor task with ID task2 not found'));
            expect(mockPrismaService.taskDependency.create).not.toHaveBeenCalled();
        });
        it('should throw BadRequestException when tasks are in different projects', async () => {
            ...
            await expect(service.create(createDto)).rejects.toThrow(new common_1.BadRequestException('Tasks must be in the same project to create a dependency'));
            expect(mockPrismaService.taskDependency.create).not.toHaveBeenCalled();
        });
        ...
        it('should throw BadRequestException for immediate circular reference', async () => {
            ...
            await expect(service.create(createDto)).rejects.toThrow(new common_1.BadRequestException('Cannot create dependency: this would create an immediate circular reference. ' +
        ...
        it('should handle Prisma unique constraint error', async () => {
            ...
            await expect(service.create(createDto)).rejects.toThrow(new common_1.ConflictException('A dependency between these tasks already exists'));
        });
    });
    describe('findAll', () => {
    ...
    describe('findOne', () => {
        ...
        it('should throw NotFoundException when dependency not found', async () => {
            ...
            await expect(service.findOne('nonexistent')).rejects.toThrow(new common_1.NotFoundException('Dependency with ID nonexistent not found'));
        });
    });
    describe('update', () => {
        const updateDto = {
...
//# sourceMappingURL=dependencies.service.spec.js.map